...
#Flash: cs.00101
#Request ID: $18DAA0F2
#Response ID: $18DAF2A0
#Broadcast ID: $18DBFEF2 

# --- Dependency Checks ---
try: import pyautogui
except ImportError: messagebox.showerror("Missing Dependency", "pyautogui not found (pip install pyautogui)"); sys.exit(1)
try: import serial; import serial.tools.list_ports
except ImportError: messagebox.showerror("Missing Dependency", "pyserial not found. Power supply control disabled."); serial = None
try: import pyperclip
except ImportError: messagebox.showerror("Missing Dependency", "pyperclip not found. Copy/Paste disabled."); pyperclip = None
try: import can
except ImportError: messagebox.showerror("Missing Dependency", "python-can not found. Direct CAN disabled."); can = None
try:
    import cv2
    from PIL import Image, ImageTk
except ImportError:
    messagebox.showwarning("Missing Dependency", "opencv-python or Pillow not found (pip install opencv-python pillow). Camera features disabled.")
    cv2 = None
    Image = None
    ImageTk = None
...
class App: 
    ...
    def __init__(self, root_tk): 
        ...
        self.root.geometry("1500x900") # Increased height slightly for PS controls 
    ...
    def initialize_can(self): 
        global can # Check if python-can module is loaded 
        if not can: 
        ...
        if not interface: 
            ...
            return False 
        if not channel and interface != 'virtual': # Virtual interface might not require a channel 
             if interface in ['serial', 'slcan']: # These definitely need a channel (COM port) 
                messagebox.showerror("CAN Error", f"CAN Channel (e.g., COM port or /dev/tty) required for '{interface}'.")
                return False 
             elif interface not in ['socketcan']: # socketcan might default, others usually need it. 
                pass # Allow empty for some, but warn if it seems wrong. 
        try:
        ...
        extra_can_args = {} 
        if interface == 'vector': 
            extra_can_args['app_name'] = 'CANoe' # Unique name - update this based on vector device manager instance.  Default name is CANoe unless user changes it.
        
        try:
            self.can_bus = can.interface.Bus(bustype=interface, channel=channel, bitrate=bitrate, receive_own_messages=False, **extra_can_args) 
            ...
            print(f"CAN Initialization Error Detail:\n{detailed_tb}") # For console debugging 
            self.log_to_status(f"ERROR: {err_msg}") 
            messagebox.showerror("CAN Initialization Error", f"{err_msg}\n\nPlease check CAN hardware, drivers, and parameters.\nSee console/status for more details.") 
            self.can_bus = None 
        finally:
            self.update_can_status_label() # Update status regardless of success/failure 
